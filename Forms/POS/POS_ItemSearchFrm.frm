...
Private Sub btnAccept_Click()
    If lvItemSearch.ListItems.Count > 0 Then
        'GET ITEM DETAILS
        GetItemExtraSellingInfo lvItemSearch.SelectedItem.SubItems(5)
        
        If isIndefiniteSize = True Then
            'SHOW CUT SIZE FORM
            Unload POS_CutSizeFrm
            POS_CutSizeFrm.Show (1)
        End If
        
        If TrackClipping = True Then
            Dim clip As Variant
            clip = MsgBox("Is this a clipping/retaso?", vbQuestion + vbYesNo)
            If clip = vbYes Then
                clip = True
            Else
                clip = False
        ...
        POS_QuantityFrm.isClipping = clip
...
'    cmd.Parameters.Append cmd.CreateParameter("@Name", adVarChar, adParamInput, 250, txtName.text)
    ...
    Set rec = ProductItemCode(txtBarcode.text)
...
'    cmd.Parameters.Append cmd.CreateParameter("@Name", adVarChar, adParamInput, 250, txtName.text)
    ...
    Set rec = ProductName(txtName.text)
