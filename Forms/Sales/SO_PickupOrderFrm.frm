        ...
        Case 2 'Save
            ...
            For Each item In lvItems.ListItems
                If item.text = "" Then
                    ...
                    If AllowNegativeInventory = False Then
                        available = CheckAvailableQuantity(item.SubItems(2))
                        
                        'CHECK IF Product Quantity Exceeded Reserves
                        Set cmd = New ADODB.Command
                        Set rec = New ADODB.Recordset
                        cmd.ActiveConnection = con
                        cmd.CommandType = adCmdStoredProc
                        cmd.CommandText = "INV_ProductReserve_Get"
                        cmd.Parameters.Append cmd.CreateParameter("@ProductId", adInteger, adParamInput, , Val(item.SubItems(2)))
                        cmd.Parameters.Append cmd.CreateParameter("@SalesOrderId", adInteger, adParamInput, , Val(SO_SalesOrderFrm.SalesOrderId))
                        cmd.Parameters.Append cmd.CreateParameter("@POS_SalesId", adInteger, adParamInput, , 0)
                        Set rec = cmd.Execute
                        If Not rec.EOF Then
                            Dim ReserveQuantity As Double
                            ReserveQuantity = rec!Quantity
                            If Val(Replace(item.SubItems(14), ",", "")) * Val(Replace(item.SubItems(5), ",", "")) > ReserveQuantity Then
                                MsgBox "Pick failed for " & item.SubItems(4) & ". Picked quantity is greater than ordered quantity.", vbCritical, "Pick failed"
                                Exit Sub
                            End If
                        Else
                            MsgBox "Cannot pick items not included in the sales order list.", vbCritical, "Pick failed"
                            Exit Sub
                        End If
                        
                        If Val(item.SubItems(15)) = 0 Then
                            MsgBox "Pick failed for " & item.SubItems(4) & ". Picked quantity is greater than ordered quantity.", vbCritical, "Pick failed"
                            Exit Sub
                        End If
                    End If
            ...
            If isFound = True Then 'update status of PO to inProgress
                ...
                With SO_SalesOrderFrm
                    ...
                    .lvSearch.SelectedItem.SubItems(2) = "Picking"
                    .StatusId = 5 'Picking
                End With
