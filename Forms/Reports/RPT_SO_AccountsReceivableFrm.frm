...
'    If chkAgingAccounts.value = 0 Then
        sql = "SELECT SO_SalesOrder.Total,SO_SalesOrder.Balance,BASE_Customer.CustomerId,BASE_Customer.CustomerCode, " & _
              "BASE_Customer.Name,BASE_Customer.MtdSales,BASE_Customer.YtdSales, SO_Invoice.Total as Invoiced " & _
              "FROM Peak.dbo.SO_SalesOrder SO_SalesOrder INNER JOIN Peak.dbo.BASE_Customer BASE_Customer ON " & _
              "SO_SalesOrder.CustomerId = BASE_Customer.CustomerId " & _
              "INNER JOIN SO_Invoice ON SO_Invoice.SalesOrderId = SO_SalesOrder.SalesOrderId " & _
              "WHERE SO_SalesOrder.Balance > 0 AND SO_SalesOrder.StatusId = 4 " & Order & Customer & Code & OrderBy
