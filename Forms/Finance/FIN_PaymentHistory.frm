      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.Frame Frame2 
            ...
            Begin VB.OptionButton optAccountsReceivable 
               BackColor       =   &H00FFFFFF&
               Caption         =   "Accounts Receivable/Cash Advance"
               BeginProperty Font 
                  Name            =   "Calibri"
                  Size            =   11.25
                  Charset         =   0
...
Private Sub btnDelete_Click()
    ...
    If x = vbYes Then
        ...
        cmd.Parameters.Append cmd.CreateParameter("@TransactionId", adInteger, adParamInput, , lvSearch.SelectedItem.SubItems(10))
        ...
        Do Until rec.EOF
            If optAccountsReceivable.value = True Then
                ...
                Set item = lvSearch.ListItems.add(, , rec!SO_PaymentHistoryId)
                    ...
                    item.SubItems(8) = FormatNumber(rec!salesdiscount, 2, vbTrue, vbFalse)
                    item.SubItems(9) = rec!Remarks
                    item.SubItems(10) = rec!TransactionId
                isAR = True
            Else
                Set item = lvSearch.ListItems.add(, , rec!PO_PaymentHistoryId)
                    ...
                    item.SubItems(6) = rec!checknumber
                    item.SubItems(7) = Format(rec!checkdate, "MM/DD/YY")
                    item.SubItems(8) = FormatNumber(rec!tax, 2, vbTrue, vbFalse)
                    item.SubItems(9) = rec!Remarks
                    item.SubItems(10) = rec!TransactionId
                isAR = False
            End If
...
Private Sub Form_Load()
    ...
    lvSearch.ColumnHeaders(9).width = lvSearch.width * 0.1225 'Tax
    lvSearch.ColumnHeaders(10).width = lvSearch.width * 0.145 'Remarks
