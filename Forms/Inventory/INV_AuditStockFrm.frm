...
Private Sub ComputeAdjustment()
    For Each item In lvItems.ListItems
        If item.SubItems(6) <> "" Then
            item.SubItems(8) = NVAL(item.SubItems(6)) - NVAL(item.SubItems(7))
...
Private Sub GetSystemQuantity()
    ...
    For Each item In lvItems.ListItems
        ...
        If Not rec.EOF Then
            Do Until rec.EOF
                item.SubItems(7) = FormatNumber(rec!Quantity, 2, vbTrue, vbFalse)
                rec.MoveNext
            Loop
        End If
    Next
...
Private Sub btnBatch_Click()
    Dim x As Variant
    x = InputBox("Input physical quantity")
    If IsNumeric(x) = True Then
        For Each item In lvItems.ListItems
            item.SubItems(7) = x
        Next
    Else
        MsgBox "Invalid quantity", vbCritical, "PeakPOS"
    End If
...
'    End If
    ...
    If AllowAccess = True Then
        ...
        If x = vbOK Then
            For Each item In lvItems.ListItems
                If item.SubItems(6) = "" Then item.SubItems(6) = item.SubItems(7)
            Next
            ComputeAdjustment
            Save (1)
            Save (2)
...
Private Sub lvItems_DblClick()
    ...
    INV_AuditStockOptFrm.txtSystem.text = lvItems.SelectedItem.SubItems(7)
    ...
    Select Case KeyCode
        ...
        Case vbKeyDelete
            ...
            If x = vbOK Then
                ...
                cmd.Parameters.Append cmd.CreateParameter("@AuditStockLineId", adInteger, adParamInput, , NVAL(lvItems.SelectedItem.SubItems(3)))
                cmd.Execute
                con.Close
            End If
        ...
        Do Until rec.EOF
            Set item = lvItems.ListItems.add(, , rec!ProductId)
                ...
                If IsNull(rec!physicalquantity) Then
                    item.SubItems(7) = ""
                Else
                    item.SubItems(7) = FormatNumber(rec!systemquantity, 2, vbTrue, vbFalse)
                End If
...
'            End If
        Case 6 'PRINT PREVIEW
            If AuditStockId <> 0 Then
                ...
                crxRpt.RecordSelectionFormula = "{INV_AuditStock.AuditStockId}= " & Val(AuditStockId) & ""
...
Private Sub Save(ByVal iStatusId As Integer, Optional isReopen As Variant)
    If lvItems.ListItems.Count > 0 Then
        ...
        For Each item In lvItems.ListItems
            ...
            cmd.Parameters.Append cmd.CreateParameter("@SystemQuantity", adDecimal, adParamInput, , NVAL(item.SubItems(7)))
            ...
            Else
                cmd.Parameters.Append cmd.CreateParameter("@PhysicalQuantity", adDecimal, adParamInput, , NVAL(item.SubItems(6)))
                                  cmd.Parameters("@PhysicalQuantity").Precision = 18
                                  cmd.Parameters("@PhysicalQuantity").NumericScale = 2
            End If
            ...
            cmd.Parameters.Append cmd.CreateParameter("@StatusId", adInteger, adParamInput, , iStatusId)
            cmd.Parameters.Append cmd.CreateParameter("@AuditLocationId", adInteger, adParamInput, , cmbAuditLocation.ItemData(cmbAuditLocation.ListIndex))
            If chkReset.value = Checked Then
                cmd.Parameters.Append cmd.CreateParameter("@ResetStockCard", adBoolean, adParamInput, , True)
            Else
                cmd.Parameters.Append cmd.CreateParameter("@ResetStockCard", adBoolean, adParamInput, , False)
            End If
