...
Begin VB.Form INV_NewStockFrm 
   ...
   Begin VB.Frame LayoutFrame_Search 
      ...
      Top             =   0
      Width           =   3615
      Begin VB.ComboBox cmbSearch_Supplier 
         BeginProperty Font 
            Name            =   "Calibri"
            Size            =   9
            Charset         =   0
...
Public Sub CountTotal()
    ...
    For Each item In lvItems.ListItems
        If item.Checked = True Then
            item.SubItems(11) = FormatNumber(NVAL(item.SubItems(10)) * ((NVAL(txtDiscount.text) / 100) * NVAL(item.SubItems(6))), 2, vbTrue, vbFalse)
        End If
        item.SubItems(12) = FormatNumber(NVAL(item.SubItems(6)) * NVAL(item.SubItems(10)) - NVAL(item.SubItems(11)), 2, vbTrue, vbFalse)
        Total = Total + NVAL(item.SubItems(12))
    Next
...
Private Sub btnSearch_Click()
    ...
    End If
    If cmbSearch_Supplier.ListIndex = 0 Then
        cmd.Parameters.Append cmd.CreateParameter("@SupplierId", adInteger, adParamInput, , Null)
    Else
        cmd.Parameters.Append cmd.CreateParameter("@SupplierId", adInteger, adParamInput, , cmbSearch_Supplier.ItemData(cmbSearch_Supplier.ListIndex))
    End If
...
Private Sub Form_Load()
    lvItems.ColumnHeaders(1).width = lvItems.width * 0.03
    lvItems.ColumnHeaders(5).width = lvItems.width * 0.0771
    ...
    lvItems.ColumnHeaders(10).width = lvItems.width * 0.0971
...
Private Sub SelectOrders()
    ...
    If Not rec.EOF Then
        ...
        If Not IsNull(rec!discount) Then
            txtDiscount.text = NVAL(rec!discount)
        Else
            txtDiscount.text = ""
        End If
        If rec!supplier <> "" Then
    ...
    If Not rec.EOF Then
        Do Until rec.EOF
            Set item = lvItems.ListItems.add(, , rec!ProductId)
                ...
                If Not IsNull(rec!discount) Then
                    item.SubItems(11) = FormatNumber(rec!discount, 2, vbTrue, vbFalse)
                End If
                item.SubItems(12) = FormatNumber(rec!subtotal, 2, vbTrue, vbFalse) 'subtotal
                item.SubItems(13) = rec!Location
                item.SubItems(14) = rec!actualquantity
            rec.MoveNext
        Loop
    End If
...
Private Sub Save(ByVal iStatusId As Integer, Optional isReopen As Variant)
    If lvItems.ListItems.Count > 0 Then
        ...
        cmd.Parameters.Append cmd.CreateParameter("@SupplierId", adInteger, adParamInput, , cmbSupplier.ItemData(cmbSupplier.ListIndex))
        cmd.Parameters.Append cmd.CreateParameter("@Discount", adDecimal, adParamInput, , NVAL(txtDiscount.text))
        ...
        For Each item In lvItems.ListItems
            ...
            cmd.Parameters.Append cmd.CreateParameter("@Discount", adDecimal, adParamInput, , NVAL(item.SubItems(11)))
                                  cmd.Parameters("@Discount").Precision = 18
                                  cmd.Parameters("@Discount").NumericScale = 2
            cmd.Parameters.Append cmd.CreateParameter("@Subtotal", adDecimal, adParamInput, , NVAL(item.SubItems(12)))
                                  cmd.Parameters("@Subtotal").Precision = 18
                                  cmd.Parameters("@Subtotal").NumericScale = 2
            cmd.Parameters.Append cmd.CreateParameter("@StatusId", adInteger, adParamInput, , iStatusId)
            cmd.Parameters.Append cmd.CreateParameter("@ActualQuantity", adDecimal, adParamInput, , item.SubItems(14))
...
'            Set item = lvSearch.ListItems.add(, , NewStockId)
'                item.SubItems(1) = txtOrderNumber.text
'                item.SubItems(2) = txtStatus.text
'                item.Selected = True
'                item.EnsureVisible
...
Public Sub Populate(ByVal data As String)
    Select Case data
        ...
        Case "Supplier"
            ...
            cmbSearch_Supplier.Clear
            cmbSearch_Supplier.AddItem ""
            cmbSearch_Supplier.ItemData(cmbSearch_Supplier.NewIndex) = 0
            ...
            If Not rec.EOF Then
                Do Until rec.EOF
                    ...
                    cmbSearch_Supplier.AddItem rec!Name
                    cmbSearch_Supplier.ItemData(cmbSearch_Supplier.NewIndex) = rec!VendorId
                    rec.MoveNext
                Loop
            End If
            ...
            On Error Resume Next
            cmbSupplier.ListIndex = 0
            cmbSearch_Supplier.ListIndex = 0
    End Select
End Sub
