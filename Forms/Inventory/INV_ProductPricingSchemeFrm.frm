        ...
        Select Case cmbOperator.text
            Case "Add"
                If cmbMode.ListIndex = 0 Then 'Percent
                    ...
                    If chkRoundOff.value = Checked Then
                        If NewPrice - Int(NewPrice) > 0 Then
                            NewPrice = Int(NewPrice) + 1
                        End If
                        item.SubItems(5) = FormatNumber(NewPrice, 0, vbTrue, vbFalse)
                    Else
...
Private Sub lvSearch_KeyDown(KeyCode As Integer, Shift As Integer)
    Select Case KeyCode
        Case vbKeyDelete
            ...
            cmd.ActiveConnection = con
            cmd.CommandType = adCmdStoredProc
            cmd.CommandText = "INV_PricingSchemeProduct_Delete"
            cmd.Parameters.Append cmd.CreateParameter("@ProductId", adInteger, adParamInput, , lvSearch.SelectedItem.text)
            cmd.Parameters.Append cmd.CreateParameter("@PricingSchemeId", adInteger, adParamInput, , cmbPricingScheme.ItemData(cmbPricingScheme.ListIndex))
            cmd.Execute
            con.Close
